...
def save_image(result, batch_size, provider, num_images, images_saved, output_path, image_callback=None):
...
def run_inference_loop(
    ...
    while images_saved < num_images:
        ...
        images_saved = save_image(result, batch_size, provider, num_images, images_saved, output_path, image_callback)
    return time_list


def run_inference_gui(
    pipeline, prompt, num_images, batch_size, image_size, num_inference_steps, guidance_scale, strength, provider, time_list, log_path
):
    import threading
...
def parse_common_args(raw_args):
    ...
    parser.add_argument("--width", default=512, type=int, help="Width of the images to generate")
    parser.add_argument("--height", default=512, type=int, help="Height of the images to generate")
...
def main(raw_args=None):
    ...
    output_path = log_path / f"{common_args.height}x{common_args.width}_step{common_args.num_inference_steps}_iter"

    image_size = (common_args.width, common_args.height)
    ...
    if not common_args.optimize:
        ...
        with warnings.catch_warnings():
            ...
            if provider == "openvino":
                ...
                pipeline = get_ort_pipeline(model_dir, common_args, ort_args, guidance_scale, image_size)
    ...
    records = []
    record = BenchmarkRecord(common_args.model_id, "fp16", package_name, "gpu", package_name, package_version)
    record.config.batch_size = common_args.batch_size
    record.config.customized["Warm Up"] = 1
    record.config.customized["Iteration"] = common_args.num_images
    record.config.customized["Width"] = common_args.width
    record.config.customized["Height"] = common_args.height
    record.config.customized["Inference Steps"] = common_args.num_inference_steps
    record.config.customized["Guidance Scale"] = common_args.guidance_scale
