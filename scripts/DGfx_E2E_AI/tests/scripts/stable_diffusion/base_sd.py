...
def main():
    ...
    if args.api == "openvino" or args.api == "openvino-nightly":
        if args.model == "v1.5" or args.model == "v2.0" or args.model == "v2.1":
            ...
            if args.precision == "int8":
                ...
                pipeline = OVStableDiffusionPipeline.from_pretrained(model_id, device="GPU", export=True, quantization_config=quantization_config, ov_config={"INFERENCE_PRECISION_HINT": "f16"})
            else:
                pipeline = OVStableDiffusionPipeline.from_pretrained(model_id, device="GPU", export=True, ov_config={"INFERENCE_PRECISION_HINT": "f16"})
        elif args.model == "lcm":
            from optimum.intel import OVLatentConsistencyModelPipeline
            if args.precision == "int8":
                from optimum.intel import OVWeightQuantizationConfig
                quantization_config = OVWeightQuantizationConfig(bits=8)
                pipeline = OVLatentConsistencyModelPipeline.from_pretrained(model_id, device="GPU", export=True, quantization_config=quantization_config, ov_config={"INFERENCE_PRECISION_HINT": "f16"})
            else:
                pipeline = OVLatentConsistencyModelPipeline.from_pretrained(model_id, device="GPU", export=True, ov_config={"INFERENCE_PRECISION_HINT": "f16"})
        elif args.model == "xl":
            from optimum.intel import OVStableDiffusionXLPipeline
            if args.precision == "int8":
                from optimum.intel import OVWeightQuantizationConfig
                quantization_config = OVWeightQuantizationConfig(bits=8)
                pipeline = OVStableDiffusionXLPipeline.from_pretrained(model_id, device="GPU", export=True, quantization_config=quantization_config)
            else:
                model_dir = f'stable-diffusion_{args.model}'
                if Path(model_dir).exists():
                    pipeline = OVStableDiffusionXLPipeline.from_pretrained(model_dir, device="GPU", export=False)
                else:
                    pipeline = OVStableDiffusionXLPipeline.from_pretrained(model_id, device="GPU", export=True)
            ...
            pipeline = OVStableDiffusionXLPipeline.from_pretrained(output_model_path, device="GPU")
            
        elif args.model == "v3.0":
            model_dir = f'stable-diffusion_{args.model}'
            from optimum.intel import OVStableDiffusion3Pipeline
            if Path(model_dir).exists():
                pipeline = OVStableDiffusion3Pipeline.from_pretrained(model_dir, device="GPU", export=False, ov_config={"INFERENCE_PRECISION_HINT": "f16"})
            else:
                print(f'pipeline: calls from_pretrained.')
                pipeline = OVStableDiffusion3Pipeline.from_pretrained(model_id, device="GPU", export=True, ov_config={"INFERENCE_PRECISION_HINT": "f16"})
                print(f'pipeline: {pipeline} from_pretrained.')
                pipeline.save_pretrained(model_dir)
                print(f'pipeline: {pipeline} save_pretrained.')

        elif args.model == "flux.1_schnell":
            from optimum.intel import OVFluxPipeline
            pipeline = OVFluxPipeline.from_pretrained(model_id, device="GPU", export=False, ov_config={"INFERENCE_PRECISION_HINT": "f16"})
    ...
    if args.demo:
        ...
        records = []
        record = BenchmarkRecord(model_id, args.precision, package_name, "gpu", package_name, package_version)
