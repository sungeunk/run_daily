...
parser.add_argument("--model", type=str, choices=["edsr"], help="Define LLM model to use")
parser.add_argument("--upscale", type=int, choices=[2, 3, 4], help="Define upscaling factor")
...
if args.model == "edsr":
    ...
    if args.api != "openvino":
        model = EdsrModel.from_pretrained(model_id, scale=args.upscale)
        model.half()
        model.to(device)
    else:
...
def transforms(examples):
    examples["pixel_values"] = ImageLoader.load_image(examples['image'][0])
    return examples
        ...
        while time.perf_counter() - start < 15:
            ...
            with torch.no_grad():
                ...
                if args.api == "openvino":
                    ...
                    logits = model(input.unsqueeze(0).half().to(device))
