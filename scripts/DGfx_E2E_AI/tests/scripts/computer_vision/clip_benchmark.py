...
parser.add_argument("--model", type=str, choices=["clip"], help="Define LLM model to use")
...
def transforms(examples):
    output = processor(text=["This is a dummy example number 1", "This is a dummy example number 2"], images=examples['image'], return_tensors="pt", padding=True)
    return output
...
def measure_perf(model, n=4):
    ...
    for run in range(n):
        ...
        while time.perf_counter() - start < 15:
            latency_run = []
            input = dataset[img_idx]
            with torch.no_grad():
                
                start_latency = time.perf_counter()
                if args.api == "openvino":
                    logits = model(input.unsqueeze(0).half())
                else:
                    input['input_ids'] = torch.vstack((input['input_ids'],input['input_ids'])).to(device)
                    input['attention_mask'] = torch.vstack((input['attention_mask'],input['attention_mask'])).to(device)
                    input['pixel_values'] = input['pixel_values'].unsqueeze(0).half().to(device)
                    logits = model(**input)
