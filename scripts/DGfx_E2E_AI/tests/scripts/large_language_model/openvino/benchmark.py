...
def main():
    ...
    try:
        ...
        f = open("results-INT4.json")
        data = json.load(f)
        
        filename = "execution_results.csv"
        package_name, package_version = get_target_pip_package_version(["openvino-genai"])
        model_info = data['metadata']['model']
        

        first_tok_latency = []
        subsequent_tok_latency = []
        wall_clock_time = []
        for idx, i in enumerate(data['perfdata']['results']):

            if idx == 0: # skip warm up run
                continue
            
            if i['first_latency'] == "":
                first_tok_latency.append(0)
            else:
                first_tok_latency.append(i['first_latency'])
                
            if i['second_avg_latency'] == "":
                subsequent_tok_latency.append(0)
            else:
                subsequent_tok_latency.append(i['second_avg_latency'])
            
            if i['generation_time'] == "":
                wall_clock_time.append(0)
            else:
                wall_clock_time.append(i['generation_time'])
        
        records = []
        record = BenchmarkRecord(model_info, data['metadata']['precision'], "openvino", data['metadata']['device'], package_name, package_version)
        record.config.batch_size = data['metadata']['batch_size']
        record.config.customized["Prompt Length"] = args.prompt_file[0]
        # record.config.customized["Tokens Generated"] = row["Tokens Generated"]
        record.config.customized["Max Length"] = args.infer_count
        record.metrics.customized["Pre Inference Time (s)"] = round(data['perfdata']['compile_time'],2)
        record.metrics.customized["First token latency (ms)"] = round(np.average(first_tok_latency), 2)
        record.metrics.customized["Second+ token latency (ms)"] = round(np.average(subsequent_tok_latency), 2)
        record.metrics.customized["Token rate (tokens/s)"] = round(data['metadata']['batch_size'] * (1 / round(np.average(subsequent_tok_latency), 2) * 1000), 2) 
        record.metrics.customized["Wall Clock throughput (tokens/s)"] = round(data['metadata']['batch_size'] * (args.infer_count / round(np.average(wall_clock_time), 2)), 2)
        record.metrics.customized["Wall Clock Time (s)"] = round(np.average(wall_clock_time), 2)
        ...
        record.import_accuracy_score("wwb_generated/metrics.csv")
        ...
        BenchmarkRecord.save_as_txt(log_path / "original_full_output.txt", [data])
