...
def run_generate(num_tokens, num_input_tokens, num_beams):
    ...
    prompt_path = str(current_path / "prompts"  / args.sub_model_name / args.input_tokens) + ".jsonl"
    print(prompt_path)
    ...
    if args.token_latency:
        ...
        records = []
        record = BenchmarkRecord(args.model_id, "int4", package_name, "gpu", package_name, package_version)
        record.config.batch_size = args.batch_size
        record.config.customized["Prompt Length"] = input_size
        # record.config.customized["Tokens Generated"] = gen_text
        record.config.customized["Max Length"] = args.max_new_tokens
        record.metrics.customized["Pre Inference Time (s)"] = round(pre_inference_time,2)
        record.metrics.customized["First token latency (ms)"] = round(first_latency * 1000, 2)
        record.metrics.customized["Second+ token latency (ms)"] = round(average_2n_latency * 1000, 2)
        record.metrics.customized["Token rate (tokens/s)"] = round(args.batch_size * (1 / round(average_2n_latency * 1000, 2) * 1000), 2) 
        record.metrics.customized["Wall Clock throughput (tokens/s)"] = round(args.batch_size * (total_new_tokens[0] / round(latency, 2)), 2)
        record.metrics.customized["Wall Clock Time (s)"] = round(latency, 2)
