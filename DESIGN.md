...
- performance 방향성/threshold 판정 로직 단일화
- `AnalysisResult` contract 기반 last-known-good 재사용 구조 정리
- baseline/last-known-good candidate query 정책 공통화
- functional issue count 명칭 정리
...
P2 bisect와 noisy dual gate를 추가하기 전에, 이미 구현된 분석/조회 경로의 중복 판정과 후처리 flow를 먼저 정리한다. 목표는 기능을 크게 바꾸지 않고 같은 의미의 계산과 저장 경로를 한 곳으로 모아 이후 확장 시 drift를 줄이는 것이다.

11. **성능 방향성/threshold 판정 단일화**
  `analysis/verdict.py`를 canonical 판정 모듈로 두고, viewer fallback 비교와 Compare 탭의 hard-coded `0.05` 판정을 공통 helper/config 기반으로 맞춘다. SQL trend query처럼 DB window 계산이 필요한 경로는 direction 상수와 threshold policy를 문서화하고, engine verdict와 의미가 달라지지 않도록 테스트를 추가한다.

12. **last-known-good을 `AnalysisResult` contract로 승격**
...
15. **정리 후 회귀 검증**
  정리 작업은 behavior-preserving 변경을 원칙으로 한다. 최소 검증은 `daily/tests/test_analysis_engine.py`와 `daily/tests/test_viewer_pipeline.py`이며, 특히 unit 방향성, fallback compare, timeout-only issue, LKG summary/mail rendering을 고정 테스트로 유지한다.

### P2 — Bisect·고도화 (M5)

16. **last known good 탐색 API**
