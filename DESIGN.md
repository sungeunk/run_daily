...
- Streamlit 조회 UI
  - `daily/viewer/app.py`에서 Dashboard, Excel, Regression, Geomean, Noise 탭을 제공한다.
...
functional_issues
  run_id, nodeid, outcome, category, message
```
...
- [ ] functional 히스토리/집계 뷰 추가
- [ ] run-to-run 직접 비교 뷰 추가
- [ ] latest run build health 카드 추가
...
8. **메일 본문 템플릿 고도화**
   `delivery.py`에 `AnalysisResult`를 받아 구조화 본문을 렌더링하는 함수를 추가한다.

9. **Streamlit functional 히스토리 뷰 추가**
   `queries.py`에 `functional_issues` 조회 함수를 추가하고, `app.py`에 탭 또는 패널을 붙인다.

10. **run-to-run 직접 비교 뷰 추가**
    `analysis_comparisons` 기반으로 두 run을 선택해 model-level 비교 테이블을 보여주는 Streamlit 뷰를 만든다.

### P2 — Bisect·고도화 (M5)

11. **last known good 탐색 API**
    `baseline.py`에 `overall_status = green` 기반 탐색 함수를 추가한다.
